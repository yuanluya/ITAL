...
def main():
    ...
    dps = 3 * dd
    num_particles = 1000
    train_iter_smart = 8000 #2500 + 2500 * (lt == 0)
    ...
    config_LS = edict({'particle_num': num_particles, 'data_dim': dd, 'reg_coef': reg_coef, 'lr': lr,
                       'num_classes': num_classes, 'noise_scale_min': 0.01, 'noise_scale_max': 0.05,
                       'noise_scale_decay': 2000, 'target_ratio': 0, 'new_ratio': 1.0, 'replace_count': 1})
    ...
    plt.legend([line0, line1, line3],#, line3S],
               ['zero', 'one',
