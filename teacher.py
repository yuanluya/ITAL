...
class Teacher:
    ...
    def update_stu_est(self, stu_gt_w, num = 10):
        for i in range(num):
            idx = np.random.randint(self.data_pool_.shape[0])
            self.stu_current_w_ -= self.config_.lr * (np.sum(self.data_pool_tea_[idx: idx + 1, :] * self.stu_current_w_) -\
                                                      np.sum(self.data_pool_[idx: idx + 1, :] * stu_gt_w)) * self.data_pool_tea_[idx: idx + 1, :]
